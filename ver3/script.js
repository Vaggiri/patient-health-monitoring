...
function getAIRecommendation(patientData) {
    ...
    const avgBpm = recentReadings.reduce((sum, r) => sum + r.bpm, 0) / recentReadings.length;
    const avgSpo2 = recentReadings.reduce((sum, r) => sum + r.spo2, 0) / recentReadings.length;
    const sleepReadings = recentReadings.filter(r => r.sleepQuality);
    const avgSleepQuality = sleepReadings.length > 0 ? (sleepReadings.reduce((sum, r) => sum + r.sleepQuality, 0) / sleepReadings.length) : null;
...
function createPatientListItem(name, data) {
    ...
    const hasFeedback = data.feedback && Object.keys(data.feedback).length > 0;
    ...
    element.innerHTML = `
        <div class="col-span-12 md:col-span-3 flex items-center">
            ...
            <div>
                <p class="font-semibold text-slate-800 capitalize flex items-center">
                    ${name}
                    ${hasFeedback ? `<span class="ml-2 w-3 h-3 bg-red-500 rounded-full" title="New feedback"></span>` : ''}
                </p>
        ...
        <div class="col-span-12 md:col-span-3 flex flex-wrap gap-2 md:justify-end items-center">
             ${hasFeedback ? `<button class="feedback-btn bg-red-100 text-red-700 px-3 py-2 rounded-lg text-sm font-medium hover:bg-red-200 transition" data-patient="${name}"><i data-feather="message-square" class="w-4 h-4"></i></button>` : ''}
...
function loadPatientDashboardData() {
    ...
    // NEW: Listener for user personal details to update calorie goal
...
function displayDailyReport(dayData, dateString) {
    ...
    const insights = {
        ...
        avgSleep: sleepQualities.length > 0 ? (sleepQualities.reduce((a, b) => a + b, 0) / sleepQualities.length).toFixed(1) : 'N/A'
    };
...
/**
 * MODIFIED: Settings page now saves user profile data instead of just calculating BMR.
 */
    ...
    feedbackRef.once('value', (snapshot) => {
        ...
        if (feedbackData) {
            const feedbackHtml = Object.entries(feedbackData).reverse().map(([timestamp, data]) => {
                return `<div class="p-2 border-b"><p class="text-sm text-slate-800">${data.text}</p><p class="text-xs text-slate-400 mt-1">${new Date(timestamp).toLocaleString()}</p></div>`;
            }).join('');
            showNotification(`Feedback for ${patientName}`, `<div class="max-h-60 overflow-y-auto custom-scrollbar">${feedbackHtml}</div>`);
        } else {
            showNotification(`Feedback for ${patientName}`, 'No feedback has been submitted by this patient.');
        }
    });
}
