...
const users = {
    'kumar@demo.com': { password: '123456', name: 'Mr. Kumar', role: 'patient' },
    'pradeep@demo.com': { password: '123456', name: 'Mr. Pradeep', role: 'patient' }, // ADDED NEW PATIENT
...
// Replace the existing sendEmailAlert function in script.js

function sendEmailAlert(patientName, alertType, readings) {
    console.warn(`--- 📧 TRIGGERING EMAIL ALERT VIA NETLIFY for ${patientName} ---`);
    ...
    // Call our serverless Netlify function
    ...
    .catch((error) => {
        ...
        showNotification('Alert Error', 'Could not trigger the email alert. Please check the connection.');
    });
}
...
function createPatientListItem(name, data) {
    ...
    element.innerHTML = `
        ...
        <div class="col-span-12 md:col-span-3 flex flex-wrap gap-2 md:justify-end items-center">
             ${data.feedback ? `<button class="feedback-btn bg-red-100 text-red-700 px-3 py-2 rounded-lg text-sm font-medium hover:bg-red-200 transition" data-patient="${name}"><i data-feather="message-square" class="w-4 h-4 mr-1 inline-block"></i> View Chat</button>` : `<button class="feedback-btn bg-slate-100 text-slate-700 px-3 py-2 rounded-lg text-sm font-medium hover:bg-slate-200 transition" data-patient="${name}"><i data-feather="message-square" class="w-4 h-4 mr-1 inline-block"></i> Open Chat</button>`}
    ...
    element.querySelector('.feedback-btn').addEventListener('click', (e) => {
        showChatModal(e.currentTarget.dataset.patient);
    });
...
function renderPatientDashboard() {
    mainContent.innerHTML = `
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 health-card">
             <h3 class="text-lg font-semibold text-slate-800 mb-2">Communication Channel</h3>
             <p class="text-sm text-slate-500 mb-4">Report symptoms or ask questions directly to your doctor.</p>
             <button id="openChatBtn" class="mt-2 bg-green-600 text-white px-4 py-2 rounded-lg font-semibold hover:bg-green-700 transition flex items-center justify-center">
...
// 2. Continuous Feedback Loop (MODIFIED FOR 2-WAY CHAT)
function submitPatientFeedback(patientName, text) {
    if (text) {
        const timestamp = new Date().toISOString().replace(/\./g, '_');
        const feedbackRef = database.ref(`patients/${patientName}/feedback/${timestamp}`);
        feedbackRef.set({ text: text, read: false })
            .then(() => {
                showChatModal(patientName); // Refresh chat modal
            })
            .catch(error => {
                showNotification('Error', 'Could not send message. Please try again.');
            });
    }
}

function showChatModal(patientName) {
    ...
    feedbackRef.once('value', (snapshot) => {
        ...
        const chatMessagesHtml = Object.entries(feedbackData)
            .sort((a, b) => a[0].localeCompare(b[0])) // Sort chronologically
            .map(([timestamp, data]) => {
                if (isDoctor && !data.read) {
                    unreadKeys.push(timestamp);
                }
                const patientMsgTime = new Date(timestamp.replace(/_/g, '.')).toLocaleString();
                
                const patientBubbleClass = isDoctor ? 'bg-slate-200 text-slate-800' : 'bg-blue-500 text-white ml-auto';
                const patientMsg = `
                    <div class="max-w-[80%] w-fit rounded-xl px-4 py-2 mb-2 ${patientBubbleClass}">
                        <p class="text-sm">${data.text}</p>
                        <p class="text-xs opacity-75 mt-1 text-right">${patientMsgTime}</p>
                    </div>`;

                let doctorReply = '';
                if (data.reply) {
                    const doctorReplyTime = new Date(data.reply.timestamp).toLocaleString();
                    const doctorBubbleClass = isDoctor ? 'bg-blue-500 text-white ml-auto' : 'bg-slate-200 text-slate-800';
                    doctorReply = `
                        <div class="max-w-[80%] w-fit rounded-xl px-4 py-2 mb-2 ${doctorBubbleClass}">
                             <p class="text-sm">${data.reply.text}</p>
                             <p class="text-xs opacity-75 mt-1 text-right">${doctorReplyTime}</p>
                        </div>`;
                }
        ...
        const chatInputSection = `
            <div class="mt-4 pt-4 border-t">
                <div class="flex items-center space-x-2">
                    <input id="chatInputText" class="w-full p-2 border rounded-lg text-sm" placeholder="Type your message...">
                    <button id="sendChatBtn" class="bg-blue-600 text-white p-2 rounded-full hover:bg-blue-700 transition flex-shrink-0">
        ...
        showNotification(
            `Chat with ${isDoctor ? 'Patient ' + patientName.charAt(0).toUpperCase() + patientName.slice(1) : 'Dr. Tamil'}`,
            `<div class="max-h-80 overflow-y-auto custom-scrollbar pr-2 mb-2 flex flex-col">${chatMessagesHtml || '<p class="text-center text-slate-500">No messages yet.</p>'}</div> ${chatInputSection}`
        );
        feather.replace();
        
        const chatContainer = document.querySelector('#modalMessage > div');
        chatContainer.scrollTop = chatContainer.scrollHeight;

        document.getElementById('sendChatBtn').onclick = () => {
            const inputText = document.getElementById('chatInputText').value.trim();
            if (inputText) {
                if (isDoctor) {
                    const latestTimestamp = Object.keys(feedbackData).sort().pop();
                    if(latestTimestamp) {
                         sendDoctorReply(patientName, latestTimestamp, inputText);
                    } else {
                        showNotification('Error', 'Cannot reply. Patient has not sent a message yet.');
                    }
                } else { // Is Patient
                    submitPatientFeedback(patientName, inputText);
                }
            }
        };

        if (isDoctor && unreadKeys.length > 0) {
            const updates = {};
            unreadKeys.forEach(key => {
                updates[`/patients/${patientName}/feedback/${key}/read`] = true;
            });
...
function sendDoctorReply(patientName, timestamp, text) {
    ...
    replyRef.set({
        ...
        showChatModal(patientName); // Refresh chat
    }).catch(error => {
